...
void HttpGlobalInit::Initilize()
{
    curl_global_init(CURL_GLOBAL_ALL);
}

void HttpGlobalInit::UnInitilize()
{
    curl_global_cleanup();
}
...
{
	//DBG_LOGA(YK_LOG_LEVEL_BASE, "httplib 异步线程启动");
	...
	while (bContinueRun)
	{
        ...
        std::vector<CURL_OBJ*> vecCurlObj;
        {
            lock_guard<std::recursive_mutex> dataMutex(m_dataMutex);
            vecCurlObj.swap(m_vecCurlObj);
        }

        for (auto& iter : vecCurlObj)
        {
            curl_multi_add_handle(m_multi_handle, iter->curl_handle);
        }

		if (dwWaitRet == 0)
		{
			bContinueRun = FALSE;
			//DBG_LOGA(YK_LOG_LEVEL_BASE, "AsyncRequestWorkThread wait exit");
			break;
		}

		while (curl_multi_perform_ex() > 0)
		{
			CURLMcode wait_code = curl_multi_wait(m_multi_handle, NULL, 0, CURL_MAX_WAIT_MSECS, NULL);

			if (wait_code != CURLM_OK)
			{
				char szBuffer[255] = { 0 };
                sprintf_s(szBuffer, "curl_multi_wait 异常 返回值：%d", wait_code);

				//DBG_LOGA(YK_LOG_LEVEL_BASE, "AsyncRequestWorkThread %s", szBuffer);
				break;
			}
	...
	//DBG_LOGA(YK_LOG_LEVEL_BASE, "AsyncRequestWorkThread httplib 异步线程退出");
	return 0;
}
...
std::string HttpLib::s_strProxyAuth;
curl_proxytype HttpLib::s_proxyType = CURLPROXY_HTTP;
...
void HttpLib::AsyncRequestImpl(RequestPtr pRequest, HTTP_REQUEST_CB http_request_cb, IHttpSupport* interface_cb)
{
    ...
    {
        lock_guard<std::recursive_mutex> dataMutex(m_dataMutex);
        m_vecCurlObj.push_back(pCurlObj);
    }
...
BOOL HttpLib::SyncDownloadFile(const char* file_url, const std::wstring::value_type* save_as, IHttpSupport* interface_cb /*= NULL*/)
{
...
void HttpLib::AsyncDownLoadFile(const char* file_url, const std::wstring::value_type* save_as, HTTP_REQUEST_CB http_request_cb)
{
	RequestPtr pRequest = HttpRequest::Create();
...
void HttpLib::AsyncDownLoadFile(const char* file_url, const std::wstring::value_type* save_as, IHttpSupport* interface_cb)
{
	RequestPtr pRequest = HttpRequest::Create();
...
void HttpLib::SetProxy(curl_proxytype proxyType, const char* proxy, const char* szAuth)
{
    s_strProxy = (proxy == nullptr) ? "" : proxy;
    s_strProxyAuth = (szAuth == nullptr) ? "" : szAuth;
    s_proxyType = proxyType;
}
...
CURL_OBJ* HttpLib::curl_object_prepare(RequestPtr pRequest, ResponsePtr pResponse)
{
	...
	{
#ifdef USE_WINDOW_FILE_FUNCTION
		pCurlObj->file_handle = ::CreateFile(
    ...
    if (pRequest->GetTimeOut() > 0)
    {
        SET_CURL_OPT(CURLOPT_TIMEOUT, pRequest->GetTimeOut());
    }
    else if(s_lTimeout > 0)
    {
        SET_CURL_OPT(CURLOPT_TIMEOUT, s_lTimeout);
    }

    SET_CURL_OPT(CURLOPT_FORBID_REUSE, 1);
	...
	if (!s_strProxy.empty())
    {
        ...
        SET_CURL_OPT(CURLOPT_PROXYTYPE, s_proxyType);
        if (!s_strProxyAuth.empty())
        {
            SET_CURL_OPT(CURLOPT_PROXYUSERPWD, s_strProxyAuth.c_str());
        }
	}
