...
{
	if( ::IsBadReadPtr( pDataSrc , nSize ) || nSize <= 0 )
	{
		ATLASSERT( FALSE );
		return ;
	}
	...
	CAutoBuffer tmpCopy;
	tmpCopy.CopyBuffer( pDataSrc , nSize );
	AllocateBuffer( tmp.GetBufferSize() + nSize );
	memcpy( GetBuffer() , tmp.GetBuffer() , tmp.GetBufferSize() );
	memcpy( GetBuffer() + tmp.GetBufferSize(), tmpCopy.GetBuffer() ,tmpCopy.GetBufferSize() );
}
...
bool CAutoBuffer::AttachBuffer( char *&pDataSrc , unsigned int nSize )
{
	FreeBuffer();
	m_pData = pDataSrc;
	m_nSize = m_pData ? nSize : 0;
	pDataSrc = NULL;
	return true;
}

void CAutoBuffer::DetachBuffer()
{
	m_pData = NULL;
	m_nSize = 0;
}

std::string CAutoBuffer::ToString()
{
	std::string strRet;
	if( m_nSize > 0 && m_pData != NULL )
...
const char& CAutoBuffer::operator[]( unsigned int i ) const
{
    if( m_pData == NULL )
	{
		throw std::out_of_range( "empty pointer" );
	}
    else if(  i >= m_nSize )
	{
